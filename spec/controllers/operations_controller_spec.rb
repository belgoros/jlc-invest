...
RSpec.describe OperationsController, type: :controller do
  ...
  before(:each) do
    ...
    @operation = create(
      ...
      withholding: 12, account: account
    )
  end
  ...
  describe 'DELETE #destroy' do
    it 'deletes the operation from the database' do
      expect {
        delete :destroy, params: {id: @operation, account_id: account.id}
      }.to change(Operation, :count).by(-1)
    end
  ...
  describe 'PDF generation' do
    it 'returns a PDF file inline' do
      get :show, params: {id: @operation, account_id: account.id, format: :pdf}
